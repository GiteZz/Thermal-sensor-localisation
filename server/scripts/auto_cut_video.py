...
scene_start = datetime.combine(date.today(), time(hour=14, minute=55, second=0))
scene_stop = datetime.combine(date.today(), time(hour=15, minute=0, second=0))
...
for i in range(scene_frame_amount):
    ret, frame = cap.read()
    img_name = 'actual_frames/' + ('000000' + str(i))[-6:] + '.png'
    print(img_name)
